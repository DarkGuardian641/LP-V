...
void mergeSortParallel(vector<int>& arr, int left, int right) {
  if (right - left <= 100) { // Threshold for sequential execution (adjust as needed)
    ...
    #pragma omp task firstprivate(arr, mid, right)
...
int main() {
  ...
  bubbleSortSequential(temp_arr);
  auto end = chrono::high_resolution_clock::now();
  double sequentialBubbleSortTime = chrono::duration_cast<chrono::microseconds>(end - start).count() / 1000.0;
  ...
  bubbleSortParallel(temp_arr);
  end = chrono::high_resolution_clock::now();
  double parallelBubbleSortTime = chrono::duration_cast<chrono::microseconds>(end - start).count() / 1000.0;
  ...
  temp_arr = original_arr;
  start = chrono::high_resolution_clock::now();
  mergeSortRecursive(temp_arr, 0, N - 1);
  end = chrono::high_resolution_clock::now();
  double sequentialMergeSortTime = chrono::duration_cast<chrono::microseconds>(end - start).count() / 1000.0;
  ...
  temp_arr = original_arr;
  start = chrono::high_resolution_clock::now();
  mergeSortParallel(temp_arr, 0, N - 1);
  end = chrono::high_resolution_clock::now();
  double parallelMergeSortTime = chrono::duration_cast<chrono::microseconds>(end - start).count() / 1000.0;
